...
<span
  ...
  ionStepContent="Bem-vindo ao tour!"
>
  Olá, tudo bem?
</span>
```
...
- `ionStepContent` é o conteúdo que será exibido na etapa do tour. Pode ser um texto ou um template customizado.
- `ionStepPrevBtnTitle` é o texto do botão de voltar para a etapa anterior. O botão só será exibido se houver uma etapa anterior;
- `ionStepNextBtnTitle` é o texto do botão de avançar para a próxima etapa. O botão só será exibido se houver uma etapa seguinte;
- `ionStepCloseBtnTitle` é o texto do botão de fechar o tour. O botão só será exibido se a etapa for a última do tour;
...
- `ionStepPosition` é a posição do balão de fala da etapa. Pode assumir 12 valores definidos pelo enum `IonTourStepPositions` ;
