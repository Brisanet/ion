...
- `dropdownConfig`. Possibilita configurar o dropdown, ao repassar as propriedades `required`, `notShowClearButton` e `title`.
...
<Canvas>
  ...
  >
    {{
      template: `<ion-button label="Primary" [options]="[{ label: 'option 1' }, {label: 'option 2'}]" [dropdownConfig]="{title: 'Example Title'}"></ion-button>`,
    }}
    secondary
  </Story>
</Canvas>
