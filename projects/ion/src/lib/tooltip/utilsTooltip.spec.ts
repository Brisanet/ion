...
describe('getPositions', () => {
  describe('with arrow pointing at center', () => {
    ...
    it.each(['bottomLeft', 'bottomRight', 'topLeft', 'topRight'])(
      ...
      (position) => {
        expect(positions[position as TooltipPosition].left).toBe(horizontalCenter);
      }
    );
  });
  describe('with arrow pointing at borders of host', () => {
    ...
    it.each(positionsAlignedToHost)(
      ...
      ({ position, pointTo }) => {
        expect(positions[position as TooltipPosition].left).toBe(hostDimensions[pointTo as keyof typeof hostDimensions]);
      }
    );
  });
});
